...
parser.add_argument('peak_sets_dir')
parser.add_argument('models_dir')
parser.add_argument('output_dir')
args = parser.parse_args()

peak_sets = sorted([x.split('.txt')[0] for x in os.listdir(args.peak_sets_dir)
             if x.endswith('.txt')])
peak_set_to_metrics = {}
for peak_set in peak_sets:
        with open(f'{args.models_dir}/{peak_set}/cv_metrics.pkl', 'rb') as f:
                    peak_set_to_metrics[peak_set] = pickle.load(f)
...
for i, metric in enumerate(metrics):
    ...
    title = titles[i]
    legend_handles = []
    for peak_set, cmap in zip(peak_sets, cmaps):
        if isinstance(cmap, str):
            cmap = plt.get_cmap(cmap)
        C_to_metric_vals = peak_set_to_metrics[peak_set]

        Cs = sorted(list(C_to_metric_vals.keys()))
        norm = mpl.colors.Normalize(vmin=-1, vmax=len(Cs))
        peak_set_colors = [cmap(norm(i)) for i in range(len(Cs))]
        ...
        for box, fc in zip(bp_dict['boxes'], peak_set_colors):
        ...
        legend_handles.append(mpl.patches.Patch(label=peak_set, color=cmap(0.3)))
    ax.legend(handles=legend_handles, bbox_to_anchor=(1, 0.5), loc='center left')
    ax.set_xticks(all_positions, Cs * len(peak_sets), rotation=60)
