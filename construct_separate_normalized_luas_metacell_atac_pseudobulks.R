...
pancan_peaks_meta <- read.table(paste0(new_out_dir, '/tcga_peaks_metadata.table'))
...
atac_counts <- atac_counts[common_peaks, , drop=FALSE]
tcga <- readRDS(paste0(new_out_dir, '/tcga_log2cpm_jointTMMwsp.rds'))

y <- DGEList(counts = atac_counts)
...
luas <- edgeR::cpm(y, log = TRUE, prior.count = prior)  # normalized.lib.sizes=TRUE by default for DGEList
...
p_overlay <- ggplot(df_long, aes(x = log2cpm, group = sample, color = cohort)) +
  ...
  labs(title = "Per-sample density of log2CPM (separate TMMwsp)",
       x = "log2CPM", y = "Density") +
  theme_classic() +
...
## ---- Collapse to aggregate distributions ----
agg <- df_long |>
  group_by(cohort) |>
  summarize(log2cpm = list(log2cpm), .groups = "drop") |>
  mutate(log2cpm = map(log2cpm, unlist))

tcga_vals <- agg$log2cpm[agg$cohort == "TCGA"][[1]]
luas_vals <- agg$log2cpm[agg$cohort == "LUAS"][[1]]

## ---- Compute reference quantiles ----
q_tcga <- quantile(tcga_vals, probs = c(0.9, 0.75), na.rm = TRUE)
q_luas <- quantile(luas_vals, probs = c(0.9, 0.75), na.rm = TRUE)

## ---- Solve for scale and shift ----
# System:  a * q_luas + b = q_tcga
A <- rbind(
  c(q_luas[1], 1),
  c(q_luas[2], 1)
)
...
## ---- Apply transform to LUAS ----
df_long <- df_long |>
  mutate(log2cpm_aligned = if_else(
    cohort == "LUAS",
    a * log2cpm + b,
    log2cpm
  ))
...
meds <- df_long |>
  group_by(sample, cohort) |>
  summarize(median_log2cpm_aligned = median(log2cpm_aligned), .groups = "drop")

ggplot(df_long, aes(x = log2cpm_aligned, group = sample, color = cohort)) +
  geom_density(size = 0.6, alpha = 0.9, adjust=3) +
  geom_vline(data = meds, aes(xintercept = median_log2cpm_aligned, color = cohort),
             linetype = "dashed", alpha = 0.5, show.legend = FALSE) +
  labs(title = "Per-sample density of adjusted log2CPM (separate TMMwsp)",
       x = "Adjusted log2CPM", y = "Density") +
  theme_classic() +
  theme(legend.position = "top")
