...
## luas_counts: peaks x clusters (raw counts), colnames like "12_3", "28_1", ...
cluster_ids <- colnames(luas_counts)
sample_key  <- sub("^([^_]+)_.*$", "\\1", cluster_ids)  # take text before first underscore

## Sum columns by sample_key: transpose -> rowsum -> transpose back
luas_sample_counts <- t(rowsum(t(luas_counts), group = sample_key))
## Rename columns to "LUAS_{sample_num}"
colnames(luas_sample_counts) <- paste0("LUAS_", colnames(luas_sample_counts))

luas_counts = luas_sample_counts
...
saveRDS(luas, file.path(new_out_dir, 'luas_sample_log2cpm_jointTMMwsp.rds'))
saveRDS(tcga, file.path(new_out_dir, 'tcga_log2cpm_jointTMMwsp.rds'))

write.table(pancan_peaks_meta, file.path(new_out_dir, 'tcga_peaks_metadata.tsv'))
